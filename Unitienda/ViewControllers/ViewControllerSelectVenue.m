...
@interface ViewControllerSelectVenue ()

@property (nonatomic, retain) UIPopoverController *popoverControllerCustom;
...
@implementation ViewControllerSelectVenue

-(void)viewDidLoad{
    [self setDetailItem:@"1pisoUnicentro"];
    _dictionaryStorePoint = [Constants GET_POSITION_DICTIONARY_ORIGIN_LAYER_AS_KEY];
}

-(void) viewDidAppear:(BOOL)animated{
    [self selectLayerWithGestureRecognizer:nil AndPoint:_point];   //Draw specific store, requeires call on this cycle method, because use the <self.contentView.layer>, and that is a children of self
}
...
#pragma mark - Handle gaps on an SVG

-(void) selectLayerWithPoint:(CGPoint)point
{
  _point = point; //Matching the point selected on Picker View with the point select on map
 [self selectLayerWithGestureRecognizer:nil AndPoint:point];
}
...
-(void) handleTapGesture:(UITapGestureRecognizer*) recognizer
{
    ...
    [self selectLayerWithGestureRecognizer:recognizer AndPoint:point];
}

-(void) selectLayerWithGestureRecognizer:(UITapGestureRecognizer*)recognizer AndPoint:(CGPoint)point
{
    ...
    if(layerForHitTesting==nil){
        NSLog(@"the layerForHitTesting is nil");
    }else{
        NSLog(@"the layerForHitTesting is not nil");
    }
    ...
    /***********************************************************
     ...
     SVGDocument* document = [[self.contentView image] DOMDocument];
     NodeList* listDOMDocument = [document childNodes];
     NSLog(@"lenght of childNodes of DOMDocument : %li", [listDOMDocument length]/2);
     
     SVGSVGElement *element = [[self.contentView image] DOMTree];
     NodeList *listDOMTree = [element childNodes];
     NSLog(@"lenght of childNodes of DOMTree : %li", [listDOMTree length]/2);
     
     SVGPolygonElement* svgPoligonElement = listDOMDocument;
     
     ************************************************************/
    
    [hitLayer setName:[NSString stringWithFormat:@"%f,%f",p.x, p.y]];
    ...
    else
    {
        ...
        if( lastTappedLayer != nil )
        {
            ...
            lastTappedLayer.borderColor = [UIColor redColor].CGColor;
            lastTappedLayer.borderWidth = 1.0;
            ...
            NSLog(@"Layer tapped id: {%.1f, %.1f} {%.1f, %.1f}", lastTappedLayer.frame.origin.x, lastTappedLayer.frame.origin.y, lastTappedLayer.frame.size.width, lastTappedLayer.frame.size.height);
            
            NSValue* nsvalueKey = [NSValue valueWithCGPoint:CGPointMake(lastTappedLayer.frame.origin.x, lastTappedLayer.frame.origin.y)];
            NSString* numbreStore = [_dictionaryStorePoint objectForKey:nsvalueKey];
            [self.delegate showStoreNumber:numbreStore];
            
            UIFont* fontToDraw = [UIFont fontWithName:@"Helvetica"
                                                 size:14.0f];
            CGSize sizeOfTextRect = [textToDraw sizeWithFont:fontToDraw];
            //            CGSize sizeOfTextRect = [textToDraw sizeWithAttributes:@{fontToDraw}];
...
- (void)setDetailItem:(id)newDetailItem {
    ...
    if (self.popoverControllerCustom) {
        [self.popoverControllerCustom dismissPopoverAnimated:YES];
    }
}
...
-(void) willLoadNewResource
{
    ...
    self.proggresLoading.progress = 0;
...
-(void) tickLoadingSVG:(NSTimer*) timer
{
    ...
    dispatch_async(dispatch_get_main_queue(),
                   ^{
                       // must be on main queue since this affects the UIKit GUI!
                       self.proggresLoading.progress = parser.currentParseRun.parseProgressFractionApproximate;
                   });
}
...
- (void)splitViewController:(UISplitViewController *)splitController willHideViewController:(UIViewController *)viewController withBarButtonItem:(UIBarButtonItem *)barButtonItem forPopoverController:(UIPopoverController *)argumentPopoverController
{
    ...
    self.popoverControllerCustom = argumentPopoverController;
}
...
- (void)splitViewController:(UISplitViewController *)splitController willShowViewController:(UIViewController *)viewController invalidatingBarButtonItem:(UIBarButtonItem *)barButtonItem
{
    ...
    self.popoverControllerCustom = nil;
}
...
- (IBAction)exportLayers:(id)sender {
    ...
    _layerExporter = [[CALayerExporter alloc] initWithView:_contentView];
...
- (IBAction)confirmVenue:(UIButton *)sender {
